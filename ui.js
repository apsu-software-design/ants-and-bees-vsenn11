...
var Vorpal = vorpal();
/**
 * Creates map including colony tunnels, bee hive, and Queen Ant location.
 * @param game
 * @returns map
 */
...
/**
 * Determines ant type and generates the appropriate icon for the map.
 * Grower: Green G
 * Thrower: Red T
 * Eater: Yellow if full, magenta otherwise
 * Scuba: Cyan S
 * Guard: _ protected ant if in use, x otherwise
 * @param ant
 * @returns icon
 */
...
/**
 * Uses Vorpal API to handle game execution.
 * Game is started with delimiter AvB $, map is then created and shown.
 * Vorpal handles user input via set commands (show, deploy, remove, boost and turn.)
 * Ant types and boost names are auto-completed for the deploy and boost commands.
 * The turn command ends the current turn.
 * The show command shows the current map.
 * The remove command removes ants from the given tunnel.
 * @param game
 */
function play(game) {
    Vorpal
